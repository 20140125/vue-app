<template>
  <div>
    <el-dialog v-model="visible" title="权限申请" :show-close="false" :close-on-click-modal="false"
      ...
      <el-form :model="localForm" :ref="reForm" label-position="left" label-width="100px" :rules="rules">
        <el-form-item label="申请人：" prop="user_id">
          <el-select filterable v-model.number="localForm.user_id" clearable
                     @change="$emit('getUserAuth', localForm.user_id)">
            <el-option v-for="(item,index) in permissionAttr.userLists" :label="item.username" :key="index"
                       :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="申请地址：" prop="href">
          <el-select filterable v-model="localForm.href" :disabled="reForm === 'update'">
            <el-option v-for="(item,index) in permissionAttr.authLists" :label="setAuthName(item)" clearable
                       :key="index" :value="item.api" :disabled="item.disable"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="申请时间：" prop="expires">
          <el-date-picker v-model="localForm.expires" clearable :editable="false" type="datetime"
                          value-format="YYYY-MM-DD HH:mm:ss" placeholder="选择申请时间"></el-date-picker>
        </el-form-item>
        <el-form-item label="申请理由：" prop="desc">
          <el-input v-model.trim="localForm.desc" maxlength="200" placeholder="请输入申请理由" show-word-limit resize="none"
                    :autosize="{ minRows: 4}" type="textarea"></el-input>
        </el-form-item>
        <SubmitButton :form="submitForm" :reForm="reForm"
                      @closeDialog="$emit('getPermissionApply', { page: 1, limit: 15, show_page: true, refresh: true })"></SubmitButton>
      </el-form>
    </el-dialog>
  </div>
</template>
