<template>
  <div id="users">
    <el-dialog v-model="visible" :title="reForm === 'created' ? '添加管理员' : '修改管理员'" :show-close="false"
      ...
      <el-form :model="localForm" :ref="reForm" label-position="left" label-width="100px" :rules="rules">
        ...
        <el-form-item label="用户角色：" prop="role_id">
          <el-select v-model.number="localForm.role_id" filterable placeholder="请选择用户角色">
            <el-option v-for="(role,index) in usersAttr['roleLists']" :key="index" :label="role.role_name"
        ...
        <SubmitButton :form="submitForm" :reForm="reForm"
                      @closeDialog="$emit('getUsersLists', { page: 1, limit: 15, refresh: true, total: 0 })"></SubmitButton>
      </el-form>
    </el-dialog>
  </div>
</template>
