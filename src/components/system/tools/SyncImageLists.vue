    ...
    <div id="imageType" v-loading="loading" element-loading-text="数据抓取中...">
        <el-form label-position="left" label-width="100px">
            <el-form-item label="关键词：">
                <el-input v-model.trim="keywords" :placeholder="`请输入图片分类地址(https://www.fabiaoqing.com/)`">
                    <template #append>
...
export default {
    ...
    methods: {
        ...
        async getWeather() {
            this.messageLists = []
            this.loading = true
            await this.$store.dispatch('UPDATE_ACTIONS', { url: URLS.spider.syncImageLists, model: { keywords: this.keywords } })
        }
    }
}
</script>
